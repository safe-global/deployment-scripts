import { type Address, defineChain, type Chain, formatEther, erc20Abi, parseEther } from "viem";
import * as fs from "fs";
import * as path from "path";
import * as dotenv from "dotenv";

// Import shared utilities
import { validateEnvironment, validateRpcUrl, validateErc20MintAmount } from "./utils/validation";
import {
...
async function deployERC20(
  ...
  try {
    ...
    if (receipt.status === "success") {
      ...
      // Mint tokens to the deployer account if contract address is available
      if (contractAddress) {
        try {
          // Get mint amount from environment with validation
          const mintAmount = validateErc20MintAmount(process.env.ERC20_MINT_AMOUNT || ENV_DEFAULTS.ERC20_MINT_AMOUNT);
          const mintAmountWei = parseEther(mintAmount);
          ...
          // Call mint function (assuming standard mint(address to, uint256 amount) signature)
          console.log("\nCalling mint function...");
          const mintHash = await client.writeContract({
            address: contractAddress,
            abi: erc20WithMintAbi,
            functionName: "mint",
            args: [account.address, mintAmountWei],
            account,
            chain: chain,
          });
          ...
          if (mintReceipt.status === "success") {
            ...
            // Optionally read the balance to verify
            try {
...
function createGitHubActionsSummary(
  ...
  const githubSummaryFile = path.join(deploymentsDir, `github-actions-summary-erc20-${networkName}-${Date.now()}.json`);
  ...
  const latestSummaryFile = path.join(deploymentsDir, `github-actions-summary-erc20-${networkName}-latest.json`);
...
async function main() {
  ...
  // Path to ERC20 JSON file
  const erc20JsonPath = path.join(
    process.cwd(),
    "contracts",
    "deployement-data",
    "tokens",
  ...
  if (!fs.existsSync(erc20JsonPath)) {
    console.error(`ERC20 JSON file not found: ${erc20JsonPath}`);
    process.exit(1);
  }

  // Read deployment data
  const deploymentData: DeploymentData = JSON.parse(
    fs.readFileSync(erc20JsonPath, "utf-8")
  );
  ...
  if (!deploymentData.data) {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("ERC20 Token Deployment Script");
    console.log("=".repeat(60));
    console.log("Deployer address:", account.address);
    console.log("Network:", networkName);
    console.log("RPC URL:", maskUrl(rpcUrl));
    console.log("ERC20 JSON path:", erc20JsonPath);
